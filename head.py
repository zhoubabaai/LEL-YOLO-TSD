...
class Segment26(Segment):
    """YOLOv8 Segment head with semantic-guided feature."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tsd_enabled = stuff_classes is not None and thing_classes is not None
        self.stuff_classes = stuff_classes
        self.thing_classes = thing_classes

        ch = list(ch) if ch and len(ch) >= 3 else [npr, npr, npr]
        ...
        # ===== TSD Branch =====
        if self.tsd_enabled:
            LOGGER.info(f"TSD-Seg enabled: Stuff{stuff_classes}, Thing{thing_classes}")

            self.ch_p3 = ch[0]
            self.ch_p4 = ch[1]
            self.ch_p5 = ch[2]

            # Stuff branch: semantic auxiliary supervision.
            self.stuff_conv = nn.ModuleList([
                nn.Sequential(
                    Conv(self.ch_p3, npr, 1),
                    Conv(npr, npr, 3),
                    nn.Conv2d(npr, len(stuff_classes) + nm, 1),
                ),
                nn.Sequential(
                    Conv(self.ch_p4, npr, 1),
                    Conv(npr, npr, 3),
                    nn.Conv2d(npr, len(stuff_classes) + nm, 1),
                ),
                nn.Sequential(
                    Conv(self.ch_p5, npr, 1),
                    Conv(npr, npr, 3),
                    nn.Conv2d(npr, len(stuff_classes) + nm, 1),
                ),
            ])

            # Thing branch: instance-level auxiliary mask coefficient head.
            # Each scale predicts nm mask coefficients per spatial position.
            self.thing_coef = nn.ModuleList([
                nn.Sequential(
                    Conv(self.ch_p3, npr, 1),
                    Conv(npr, npr, 3),
                    nn.Conv2d(npr, nm, 1),
    ...
    def forward(self, x, embed=None, **kwargs):
        if not self.tsd_enabled:
            return super().forward(x, embed=embed, **kwargs)
        ...
        for i, feat in enumerate([P3, P4, P5]):
            sem_f = self.sem_conv[i](feat)
            sem_feats.append(sem_f)

            # Residual semantic guidance.
            fused = feat + self.sem_alpha * sem_f
        ...
        # ===== Stuff Branch =====
        stuff_outs = [
            self.stuff_conv[0](sem_feats[0]),
            self.stuff_conv[1](sem_feats[1]),
            self.stuff_conv[2](sem_feats[2]),
        ]

        # ===== Thing Instance Branch =====
        # Shape after concat:
        # [B, sum(H_i * W_i), nm]
        thing_coef = torch.cat(
            [
                self.thing_coef[i](feats[i]).flatten(2)
                for i in range(3)
            ],
            dim=2,
        ).permute(0, 2, 1).contiguous()

        # Official YOLOv8 proto.
        protos = self.proto(P3)

        # Main YOLO instance segmentation branch.
        preds = self.forward_head(feats, self.cv2, self.cv3, self.cv4)

        preds["proto"] = protos
        preds["feats"] = feats
        preds["stuff"] = stuff_outs
        preds["thing"] = thing_coef

        if self.training:
            return preds

        outputs = self._inference(preds)
        return ((outputs, protos), preds)
class OBB(Detect):
